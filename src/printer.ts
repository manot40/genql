...
export function print(ast: ASTNode, options: PrintOptions = {}): string {
  const visitor = Object.fromEntries(
    Object.entries(printDocASTReducer(options)).map(([k, v]) => [k, { leave: v }])
  );
...
const printDocASTReducer = ({
  ...
  OperationDefinition(node) {
    ...
    return prettify(code, 'skip');
  },

  VariableDefinition: ({ variable, type, defaultValue, directives }) => {
    let typeUnwrapped = type?.replace('[', '').replace(']', '').replace('!', '').trim();
    ...
    return 'let ' + variable.replace('$', '') + (defaultValue ? ' = ' + defaultValue : '');
  },
